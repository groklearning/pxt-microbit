...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        (function (Util) {
            ...
            Util.cpuUs = function () { return Date.now() * 1000; };
...
(function (pxt) {
    ...
    (function (gallery) {
        ...
        function parseExampleMarkdown(name, md) {
            ...
            src = pxt.tutorial.patchArcadeSnippets(src);
...
(function (pxt) {
    ...
    (function (tutorial) {
        ...
        function parseTutorialMarkdown(tutorialmd) {
            ...
            if (metadata && metadata.activities) {
                ...
                if (!steps || steps.length <= 1)
                    steps = parseTutorialSteps(tutorialmd, _h3Regex);
        ...
        */
         ...
         */
        function patchArcadeSnippets(inputJS) {
            if (pxt.appTarget.id !== "arcade" && pxt.appTarget.id !== "pxt-32")
                return inputJS;
            var declRegex = /enum\s+SpriteKind\s*{((?:[^}]|\s)+)}/gm;
            var builtin = ["Player", "Projectile", "Enemy", "Food"];
            var match = declRegex.exec(inputJS);
            if (match) {
                var referencedNames = match[1].split(/(?:\s|,)+/)
                    .map(function (n) { return n.trim(); })
                    .filter(function (n) { return /[a-zA-Z]+/.test(n); })
                    .filter(function (n) { return builtin.indexOf(n) === -1; });
                if (referencedNames.length) {
                    return inputJS.replace(declRegex, "\nnamespace SpriteKind {\n" + referencedNames.map(function (rn) { return "    export const " + rn + " = SpriteKind.create()"; }).join("\n") + "\n}\n");
                }
                else {
                    return inputJS.replace(declRegex, "");
                }
        ...
        tutorial.patchArcadeSnippets = patchArcadeSnippets;
...
(function (pxtmelody) {
    var MelodyGallery = /** @class */ (function () {
        ...
        MelodyGallery.prototype.mkButton = function (sample, i, width, height) {
            ...
            galleryItem.addEventListener(pxt.BrowserUtils.pointerEvents.move, function () {
            ...
            galleryItem.addEventListener(pxt.BrowserUtils.pointerEvents.leave, function () {
            ...
            clickableArea.addEventListener("click", function () { return _this.handleSelection(sample); });
            ...
            var melodyName = document.createElement('div');
            ...
            var colorBlock = this.createColorBlock(sample);
            ...
            var preview = this.createPreviewButton(sample, i);
        ...
        MelodyGallery.prototype.togglePlay = function (sample, i) {
            var button = document.getElementById("play-button-" + i);
        ...
        MelodyGallery.prototype.createPreviewButton = function (sample, i) {
            var _this = this;
            ...
            preview.addEventListener("click", function () { return _this.togglePlay(sample, i); });
            return preview;
        };
...
(function (pxtsprite) {
    ...
    var CanvasGrid = /** @class */ (function () {
        function CanvasGrid(palette, state, lightMode) {
