...
(function (pxt) {
    ...
    function simpleCompileAsync(files, isNative) {
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function jsEmit(bin) {
            ...
            jssource += pxtc.U.values(ctx.framectors).join("\n") + "\n";
            ...
            function emitProcCall(topExpr) {
                ...
                if (proc)
                    ...
                    if (!ctx.framectors[id]) {
                        var locals = pxtc.U.range(topExpr.args.length).map(function (i) { return "arg" + i; });
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        ; r0 is object
                */
                for (var _b = 0, _c = Object.keys(pxtc.thumbCmpMap); _b < _c.length; _b++) {
                    var op = _c[_b];
                    op = op.replace(/.*::/, "");
    ...
    (function (pxtc) {
        ...
        function compileBinary(program, host, opts, res, entryPoint) {
            ...
            function pruneMethodsAndRecompute() {
                ...
                for (var _i = 0, functionInfos_1 = functionInfos; _i < functionInfos_1.length; _i++) {
                    var fi = functionInfos_1[_i];
            ...
            function emitObjectLiteral(node) {
                ...
                node.properties.forEach(function (p) {
                    ...
                    var keyName = p.name.kind == pxtc.SK.StringLiteral ?
            ...
            function isUsed(decl) {
                if (isOnDemandDecl(decl)) {
                    return !!(pxtInfo(decl).flags & 8 /* IsUsed */);
                }
                else {
                    return true;
                }
            ...
            */
            ...
            function emitFuncCore(node) {
                ...
                if (existing) {
                    ...
                    var pinfo_1 = pxtInfo(node);
                    proc = new pxtc.ir.Procedure();
                    proc.isRoot = !!(pinfo_1.flags & 1 /* IsRootFunction */);
                    proc.action = node;
                    proc.info = info;
                    pinfo_1.proc = proc;
                    bin.addProc(proc);
        ...
        (function (service_1) {
            ...
            var operations = {
                ...
                decompile: function (v) {
                    return pxtc.decompile(v.options, v.fileName, false);
                },
